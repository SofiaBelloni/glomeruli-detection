...
def generate_data():
    ...
    X_train = data_aug_impl_no_label(X_train, 5)
...
def train_model(model, X_train,  epochs, batch_size, X_validation=None,):
    ...
    model_checkpoint = ModelCheckpoint(
        f'../../saved_model/autoencoderv{version}_dense_{latent_space_dim}.h5', save_best_only=True, monitor='val_loss', mode='min')
...
def main():
    ...
    autoencoder, history = train_model(
        autoencoder, X_train, epochs=500, batch_size=8, X_validation=X_validation)
